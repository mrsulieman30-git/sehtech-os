...
const form = ref({
    ...
    due_date: props.bill.due_date ? dayjs(props.bill.due_date).format('YYYY-MM-DD') : '',
    notes: props.bill.notes || '',
    is_recurring: !!props.bill.is_recurring,
    recurring_frequency: props.bill.recurring_frequency || ''
});
...
const submit = async () => {
    ...
    try {
        await axios.put(`/api/finance/bills/${props.bill.id}`, form.value);
        toastStore.showToast('Vendor bill updated successfully!', 'success');
        window.dispatchEvent(new CustomEvent('refresh-finance-dashboard'));
        emit('close');
    } catch (e: any) {
        toastStore.showToast('Failed to update vendor bill.', 'error');
    } finally {
    ...
    <div class="bg-white rounded-xl w-[500px] max-w-[95vw] shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-200">
        <div class="h-[56px] px-6 border-b border-shell-border flex items-center justify-between shrink-0 bg-slate-50">
            <div class="flex items-center gap-2">
                <PhPencilSimple :size="20" class="text-dept-finance-main" />
                <h3 class="text-[15px] font-bold text-text-primary">Edit Vendor Bill</h3>
            </div>
